...
class SettingController extends BaseController {
	...
	public function actionIndex() {
        ...
        if ($data = Setting::getSetting($this->dataOwnerUid(), 'shop_')) {
        	$data['logoItem'] = $data['shop_logo'] ? Tools::format_array(File::find()->select(['id','file_url'])->where(['id' => $data['shop_logo']])->one(), ['file_url' => ['implode', ['', [Config::instance()->web_url, '###']], 'array']]) : (object)[];
        	$data['defaultImgItem'] = $data['shop_default_img'] ? Tools::format_array(File::find()->select(['id','file_url'])->where(['id' => $data['shop_default_img']])->one(), ['file_url' => ['implode', ['', [Config::instance()->web_url, '###']], 'array']]) : (object)[];
